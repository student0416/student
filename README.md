...
<body>
  ...
  <div id="question">ここに意味が表示されます</div>
  <div id="options"></div>
  <div id="result"></div>
  ...
  { vocab: 'をりふし', meaning: 'ちょうどその時' }
    ...
    function getUniqueVocabChoices(correctWord) {
      const uniqueMeanings = [...new Set(words.map(w => w.meaning))];
      const usedMeanings = [correctWord.meaning];
      const distractors = [];

      for (let i = 0; i < words.length && distractors.length < 2; i++) {
        const w = words[i];
        if (w.vocab !== correctWord.vocab && !usedMeanings.includes(w.meaning)) {
          distractors.push(w.vocab);
          usedMeanings.push(w.meaning);
        }
      }
      return shuffleArray([correctWord.vocab, ...distractors]);
    }

    function displayQuestion() {
      const wordObj = questionPool[currentQuestion];
      document.getElementById('question').textContent = `「${wordObj.meaning}」という意味を持つ語は？`;

      const options = getUniqueVocabChoices(wordObj);
